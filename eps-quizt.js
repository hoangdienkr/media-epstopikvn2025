...
Array.from(ul.children).forEach(x=>x.style.pointerEvents="none");if(finishedCount===quizData.length)showResult(quizData.length);});ul.appendChild(li)});box.appendChild(ul);box.appendChild(transBlock);container.appendChild(box)});$(".play").audioPlayer1()}
