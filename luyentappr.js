 (function($) {
    $.fn.epsQuiz = function(options) {
        ...
        function fetchLesson() {
            ...
            if (!sessionStr) {
                container.innerHTML = "⚠️ Chưa đăng nhập!";
                return;
            }
            const session = JSON.parse(sessionStr);
            if (!session.token) {
                container.innerHTML = "⚠️ Không tìm thấy token!";
                return;
            }
            ...
            $.ajax({
                ...
                success: (res) => {
                    if (!res.success || !res.data) {
                        container.innerHTML = "⚠️ Không có dữ liệu từ server!";
                        return;
                    }
                ...
                error: () => {
                    container.innerHTML = "⚠️ Lỗi kết nối server!";
                }
            });
        }
